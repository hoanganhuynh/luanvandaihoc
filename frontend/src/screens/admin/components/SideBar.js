         ...
         <ProSidebar style={{backgroundColor:'red !important'}} >
            <SidebarHeader>
               <div className='d-flex justify-content-center py-3 flex-column align-items-center'>
                  ...
                  <p className='my-2 h5 text-white'>{userInfo && userInfo.name}</p>
                  <strong className='text-capitalize text-left'
                        style={{fontSize:'11px', color:'#fafafa'}}
                  >
                     <SecurityIcon className='mr-1' style={{width:'14px', fill:'#fafafa'}}/>
            ...
            <Menu iconShape="circle">
               ...
               <MenuItem icon={<PersonIcon />}>Người dùng
                  <Link to="/admin/userlist" />
               </MenuItem>

               <MenuItem icon={<AssignmentIcon />}>Đơn hàng
                  <Link to="/admin/orderlist" />
               </MenuItem>

               <SubMenu title="Sản phẩm" icon={<FilterVintageIcon />}>
                  <MenuItem>Danh sách<Link to="/admin/productlist" /></MenuItem>
                  <MenuItem>Thêm sản phẩm<Link to="/admin/product/create" /></MenuItem>
               </SubMenu>

               <MenuItem icon={<PersonIcon />}>Nhà cung cấp
                  <Link to="/admin/supplierlist" />
               </MenuItem>

               <MenuItem icon={<CategoryIcon />}>Danh mục
                  <Link to="/admin/categorieslist" />
               </MenuItem>
               <MenuItem icon={<NatureIcon />}>Danh mục con
                  <Link to="/admin/subcategorieslist" />
               </MenuItem>

               <MenuItem icon={<LoyaltyIcon />}>Sale
                  <Link to="/admin/salelist" />
               </MenuItem>

               <MenuItem icon={<ConfirmationNumberIcon />}>Mã giảm giá
